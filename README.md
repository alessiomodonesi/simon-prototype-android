...
*   **Script di Importazione SQL**: Per facilitare la correzione e consentire di testare immediatamente l'applicazione (LazyColumn, scroll orizzontale, troncamento testi, Detail Screen) senza dover giocare per un'ora, nella cartella `/doc` del progetto è presente il file [import_test_data.sql](/doc/import_test_data.sql) che contiene lo script SQL per importare manualmente un set esteso di partite di prova.
